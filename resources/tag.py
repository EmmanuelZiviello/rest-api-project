...
class TagsInStore(MethodView):
    ...
    def post(self,tag_data,store_id): #data va sempre dopo self 
        tag= TagModel(**tag_data,store_id=store_id) #prende lo store_id dal url perchè in tagschema è opzionale che ci sia
        #if TagModel.query.filter(TagModel.store_id ==store_id,TagModel.name == tag_data["name"]).first()
        #abort(400,message="A tag with that name exists in this store.")
        #QUESTO PRIMA DELLA PRIMA RIGA SE SI VUOLE CONTROLLARE CHE OGNI TAG SIA UNIVOCO DI NOME SOLO PER NEGOZIO,NON TOTALMENTE,e nel model non sarà unique=true
        try:
            db.session.add(tag)
            db.session.commit()
...
class LinkTagsToItem(MethodView):
    ...
    def delete(self,item_id,tag_id):
        ...
        try:
            ...
            abort(500,message="Some error occured while removing the tag.")
